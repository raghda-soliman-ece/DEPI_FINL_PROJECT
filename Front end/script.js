/* ============================================
   ...
   Connected to: API via config.js
============================================ */

// ========== CHAT BOT (inside script.js so it loads on EVERY page) ==========
function initJumiaChat() {
    if (!document.body) return;
    if (document.getElementById('jumia-chat-panel')) return;

    // If page already has a static button, remove it so we rebuild button+panel together
    const oldBtn = document.getElementById('jumia-chat-btn');
    ...
    panel.innerHTML =
      '<div class="jumia-chat-head"><span>\uD83E\uDD16 \u0645\u0633\u0627\u0639\u062f \u062c\u0648\u0645\u064a\u0627</span>' +
      '<button type="button" id="jumia-chat-close">\u00d7</button></div>' +
    ...
    addBubble('\u0623\u0647\u0644\u0627\u064b! \u0623\u0646\u0627 \u0645\u0633\u0627\u0639\u062f \u062c\u0648\u0645\u064a\u0627. \u0627\u0633\u0623\u0644 \u0639\u0646 \u0627\u0644\u0634\u062d\u0646\u060c \u0627\u0644\u0637\u0644\u0628\u0627\u062a\u060c \u0627\u0644\u062f\u0641\u0639\u060c \u0623\u0648 \u0627\u0644\u0643\u0648\u0628\u0648\u0646\u0627\u062a.', 'bot');
    ...
    panel.querySelector('#jumia-chat-close').addEventListener('click', function () {
...
try {
    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', initJumiaChat);
    } else {
        initJumiaChat();
    }
    window.addEventListener('load', initJumiaChat);
} catch (e) {}

window.initJumiaChat = initJumiaChat;

// ========== USER SESSION ==========
let currentUser = JSON.parse(localStorage.getItem('jumia-user') || 'null');

function updateHeaderUser() {
    const loginLinks = document.querySelectorAll('.login-link, #login-header-link, .user-actions a[href="login.html"]');

    if (currentUser) {
        loginLinks.forEach(el => {
            el.innerHTML = `<i class="far fa-user-circle"></i> <span>${currentUser.firstName || 'حسابي'}</span>`;
...
function dedupeProductsByName(products) {
    const map = new Map();
    products.forEach(p => {
        if (!hasValidProductImage(p)) return;
...
async function fetchProducts(params = {}) {
    ...
    return list.filter(hasValidProductImage);
}
...
async function addToCart(btn, name, price, forceId = null) {
    ...
    if (existing) {
        ...
        if (imgEl?.src) existing.img = imgEl.src;
    } else {
...
/** Add current product details item to cart, then go to cart page */
async function buyNow() {
    const prod = JSON.parse(localStorage.getItem('jumia-current-product') || 'null');
    const titleEl = document.querySelector('.product-meta h1');
    const priceEl = document.querySelector('.detail-price');
    const name = (prod && prod.title) || titleEl?.textContent?.trim() || 'منتج';
    const priceText = (prod && prod.price) || priceEl?.textContent || '0';
    const numPrice = parseInt(String(priceText).replace(/[^\d]/g, ''), 10) || 0;
    const pId = prod && prod.id != null ? Number(prod.id) : null;

    const dummyBtn = document.createElement('button');
    await addToCart(dummyBtn, name, numPrice, pId);

    const mainImg = document.getElementById('main-product-img');
    const item = cartItems.find(i => (pId && i.productId === pId) || i.name === name);
    if (item && mainImg?.src) {
        item.img = mainImg.src;
        localStorage.setItem('jumia-cart', JSON.stringify(cartItems));
    }
...
document.addEventListener('click', function (e) {
    ...
    if (cardLink && /product[-_]details\.html/i.test(cardLink.getAttribute('href') || '')) {
...
async function loadProductDetails() {
    ...
    if (!/product[-_]details\.html/i.test(path)) return;
    ...
    const descEl = document.querySelector('.product-meta .description')
        || document.querySelector('.product-meta p')
    ...
    // Phone-only static specs must not stay under a Barbie / other product
    if (specTable) specTable.style.display = 'none';

    if (mainImg) {
        mainImg.src = resolveImageUrl(prodData.img);
        mainImg.alt = prodData.title || '';
    }
    if (titleEl) titleEl.textContent = prodData.title;
    if (priceEl) priceEl.textContent = prodData.price;
    if (oldPriceEl) {
        oldPriceEl.textContent = prodData.oldPrice || '';
        oldPriceEl.style.display = prodData.oldPrice ? '' : 'none';
    }
    if (discountEl && prodData.oldPrice && prodData.price) {
        const np = parseInt(String(prodData.price).replace(/[^\d]/g, ''), 10) || 0;
        const op = parseInt(String(prodData.oldPrice).replace(/[^\d]/g, ''), 10) || 0;
        if (op > np) {
            discountEl.textContent = `-${Math.round((1 - np / op) * 100)}%`;
    ...
    if (descEl) {
        descEl.textContent = prodData.description || prodData.title || '';
    }
...
(async function init() {
    ...
    if (typeof window.initJumiaChat === 'function') {
        window.initJumiaChat();
    }
})();
